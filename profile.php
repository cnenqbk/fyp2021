...
<html lang="en-US">
  ...
  session_start();
$id=$_SESSION['id'];
$query=mysqli_query($db,"SELECT * FROM member where member_id='$id'")or die(mysqli_error());
...
if (!preg_match("#.*^(?=.{8,20})(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).*$#", $pwd)){
	...
	');;
      ...
      $query = "UPDATE member SET username = '$username', password = '$password', full_name = '$fullname',
